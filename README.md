...
- [ ] 사용자로부터 자동차 이름을 입력받아 현재 게임 정보에 저장한다.
  - `,`로 이름을 구분한다.
  - 자동차 이름은 5자 이하만 가능하다.
  - input이 빈 값인지 확인한다.
  - ,,와 같이 잘못된 입력인지 여부를 확인한다.
- [ ] 사용자의 입력은 유효여부를 검증한다.
  - 유효하지 않은 경우에는 `alert`을 이용해 메시지를 보여준다.
  - 사용자가 다시 입력할 수 있도록한다.(`focus`)
- [ ] 사용자로부터 시도횟수를 입력받는다.
  - 자동차 이름이 등록되었는지 확인한다.
  - 올바른 횟수(양의 정수만 입력)를 입력했는지 확인한다.
- [ ] 확인 버튼 클릭시 게임을 시작한다.
- [ ] 시도 횟수만큼 각 자동차별로 랜덤 값을 추출(`0~9`)한다.
- [ ] 시도 횟수만큼 자동차목록에 대한 전진을 수행한다.
  - 랜덤 추출한 값이 4이상일 경우 해당 자동차를 전진시킨다.
  - 시도횟수 마다 자동차 목록과 전진여부를 표시(`-`)한다.
- [ ] 모든 전진이 끝난 후 우승 자동차를 표시한다.
  - 우승 자동차가 여럿일 경우에는 쉼표(,)를 이용하여 구분하여 표시한다
